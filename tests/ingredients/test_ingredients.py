...
class TestInredients:
    def test_get_inredients_arrey_status_code_schema(self):
        ...
        assert (
            validate(response.json(), schema=valid_schema_ingridients_array) is None
        ), "Response body not validate"

    def test_get_inredients_detail_status_code_schema(self, get_random_inredient_id):
        response = (
            client()
        ...
        assert (
            validate(response.json(), schema=valid_schema_ingridients) is None
        ), "Response body not validate"
