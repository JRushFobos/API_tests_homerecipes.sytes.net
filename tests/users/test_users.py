...
class TestUsers:
    ...
    def test_get_users_list_status_code_schema(self):
        ...
        assert (
            validate(response.json()["results"], schema=valid_schema_users_array)
            is None
        )
    ...
    def test_post_change_users_password_status_check_login(self, get_data_for_login):
        ...
        response = (
            client().set_headers(headers).post(f"/api/users/set_password/", data=body)
        )
        assert (
            response.status_code == 204
        ), f"Status not 204, current status: {response.status_code}"
        login_body = {"email": get_data_for_login["email"], "password": new_password}
